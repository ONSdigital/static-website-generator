...
import tojsonFilter from "./filters/tojsonFilter.js";
...
let designSystemVersion = fs.readJsonSync(
  `${designSystemPath}/package.json`
).version;
if (designSystemVersion === "3.0.1") {
  const projectPackageJson = fs.readJsonSync(`${process.cwd()}/package.json`, {
    encoding: "utf8",
  });
  const designSystemDependency =
    projectPackageJson.dependencies["@ons/design-system"];
...
export default async function createRenderer(data, setupNunjucks = null) {
  const searchPaths = [
    ...data.site.templateSearchPaths,
    `${designSystemPath}`,
    `${designSystemPath}/src`,
  ];
  ...
  nunjucksEnvironment.addGlobal(
    "designSystemCdnBaseUrl",
    designSystemCdnBaseUrl
  );
  nunjucksEnvironment.addGlobal(
    "designSystemCdnUrl",
    designSystemCdnBaseUrl + designSystemVersion
  );
  ...
  nunjucksEnvironment.addFilter(
    "isInternalUrl",
    createIsInternalUrlFilter(data)
  );
  nunjucksEnvironment.addFilter(
    "itemsList_from_navigation",
    itemsList_from_navigationFilter
  );
  nunjucksEnvironment.addFilter(
    "itemsList_from_navigationItems",
    itemsList_from_navigationItemsFilter
  );
  nunjucksEnvironment.addFilter(
    "localize",
    createLocalizeFilter(data.site, data.stringsByLanguage)
  );
  ...
  nunjucksEnvironment.addFilter("tojson", tojsonFilter);
  ...
  return {
    ...
    async render(page, context = {}) {
      try {
        ...
        const renderError = new Error(
          `A problem occurred whilst rendering page '${
            data?.site?.baseUrl ?? "/"
          }${page.uri}'\n\nlayout: ${page.layout}\nsite: ${
            data?.site?.name ?? "(undefined)"
          }`
        );
    ...
    async renderString(templateString, context = {}) {
      try {
        return nunjucksEnvironment.renderString(templateString, {
          ...data,
          ...context,
        });
      } catch (err) {
        const renderError = new Error(
          `A problem occurred whilst rendering string for site '${
            data?.site?.name ?? "(undefined)"
          }'.`
        );
        renderError.name = "RenderError";
